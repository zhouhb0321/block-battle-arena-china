...
// SRS标准踢墙表（完整8个状态，含顺时针和逆时针旋转）
const SRS_KICK_TABLE = [
  // 顺时针旋转 (0->R, R->2, 2->L, L->0)
...
const SRS_I_KICK_TABLE = [
  ...
  [ { x: 0, y: 0 }, { x: 2, y: 0 }, { x: -1, y: 0 }, { x: 2, y: 1 }, { x: -1, y: -2 } ], // R->2
  [ { x: 0, y: 0 }, { x: 2, y: 0 }, { x: -1, y: 0 }, { x: 2, y: 1 }, { x: -1, y: -2 } ], // 2->L
  [ { x: 0, y: 0 }, { x: -2, y: 0 }, { x: 1, y: 0 }, { x: -2, y: -1 }, { x: 1, y: 2 } ], // L->0
  // 逆时针旋转 (R->0, 2->R, L->2, 0->L)
  [ { x: 0, y: 0 }, { x: -1, y: 0 }, { x: 2, y: 0 }, { x: -1, y: 2 }, { x: 2, y: -1 } ], // R->0
